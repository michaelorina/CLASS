...
printf(" Customers \t RN(Arr) \t IAT \t AT_on_CLOCK \t RN(Service) \t Service_Time \t Time_Service_Begin \t Time_Service_End \t Customer_In_Queue \t Server_Idle_Time \t Delay");
...
for (j=0; j<=run; j++){
    ...
    printf(" \n %d \t %d \t %d \t %f \t %d \t %f \t %f \t %f \t %d \t %d \t %d", &j + 1, &RN_Arr[j], &IAT[j], &ATC[j], &RN_Ser[j], &ST[j], &TS_Begin[j], &TS_End[j], &Customer_Queue[j], &Idle[j], &Delay[j] );
